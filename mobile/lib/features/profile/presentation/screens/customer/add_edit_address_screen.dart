...
class _AddEditAddressScreenState extends State<AddEditAddressScreen> {
  ...
  // Local state for tracking selections (lists are in provider)
  ...
  void initState() {
    ...
    // Clear previous location data to avoid stale lists
  ...
  Future<void> _loadInitialData() async {
    ...
    try {
      ...
      if (countries.isNotEmpty && _selectedCountryId == null) {
        if (mounted) {
          setState(() {
            _selectedCountryId = countries.first['id'].toString();
          });
        }
      }

      if (_selectedCountryId != null) {
        await provider.loadCities(_selectedCountryId!);
      }

      if (_selectedCityId != null) {
        await provider.loadDistricts(_selectedCityId!);
      }

      if (_selectedDistrictId != null) {
        await provider.loadLocalities(_selectedDistrictId!);
      }
    } catch (e) {
  ...
  Future<void> _onCountryChanged(String? value) async {
    ...
    setState(() {
      ...
      // Lists cleared in provider when loading new ones, or we just rely on provider state update
    });
    if (value != null) {
      await context.read<AddressProvider>().loadCities(value);
    }
  }
  ...
  Future<void> _onCityChanged(String? value) async {
    ...
    setState(() {
      ...
      _selectedDistrictId = null;
      _selectedLocalityId = null;
      // Lists cleared in provider
    });
    if (value != null) {
      await context.read<AddressProvider>().loadDistricts(value);
    }
  }
  ...
  Future<void> _onDistrictChanged(String? value) async {
    ...
    if (value != null) {
      await context.read<AddressProvider>().loadLocalities(value);
    }
  }
  ...
  Future<void> _saveAddress() async {
    ...
    // Loading state handled by provider, but we can check provider.top-level loading or just await
    final provider = context.read<AddressProvider>();
    ...
    try {
      ...
      await provider.saveAddress(data, id: widget.address?.id);
    ...
    return Scaffold(
      ...
      body: Consumer<AddressProvider>(
        builder: (context, provider, child) {
          ...
          final localities = provider.localities;
          final isLoading = provider.isLoading;
          final isLoadingLocations = provider.isLoadingLocations;
                          ...
                          children: [
                            ...
                            SizedBox(
                              ...
                              child: ElevatedButton(
                                onPressed: (isLoading || isLoadingLocations)
                                    ? null
                                    : _saveAddress,
