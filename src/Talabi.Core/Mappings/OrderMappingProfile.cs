        ...
        CreateMap<Order, OrderDetailDto>()
            ...
            .ForMember(dest => dest.Items, opt => opt.MapFrom(src => src.OrderItems.Select(oi => new OrderItemDetailDto
            {
                ...
                SelectedOptions = !string.IsNullOrEmpty(oi.SelectedOptions)
                    ? System.Text.Json.JsonSerializer.Deserialize<List<CartItemOptionDto>>(oi.SelectedOptions,
                        (System.Text.Json.JsonSerializerOptions?)null)
                    : null
            }).ToList()))
            .ForMember(dest => dest.StatusHistory, opt => opt.MapFrom(src => src.StatusHistory.Select(sh =>
